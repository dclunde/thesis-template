...
\centering
  \begin{tikzpicture}[node distance = 2cm, auto]
    % Place nodes
        ...
        \node [block_pic, below of=init, node distance=3cm] (density) {Calculate Charge Density};
        \node [block_pic, below of=density, node distance=3cm] (poisson) {Solve Poisson's Equation};
        \node [block_pic, below of=poisson, node distance=3cm] (e_field) {Calculate Electric Field};
        \node [block_pic, left of=init, node distance=3cm] (velocity) {Update Particle Velocity};
        \node [block, below of=e_field, node distance=3cm] (stop) {Stop Simulation};
        % Draw edges
        \path [line] (init) -- (stop);
    \end{tikzpicture}
    \label{fig:pic_flow}
